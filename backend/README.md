...
| 조회 API | `/api/competitions`(+`/:ref`) · `/api/teams`(+`/:ref`) · **`/api/matches`(+`/:ref`) · `/api/standings`** (09-07) · **`/api/players/:ref` · `/api/stats/{scorers,assisters}`** (09-08 PR #34). Swagger `/docs` 가 계약 |
| 백업 | `pg_dump` 주기 백업 + 복원 리허설 (로컬 보관 · 수동 실행) |
| 테스트 | 단위 53 · e2e 10파일 82건 (`l0`·`l1`·`l2`·`l6` 는 로컬 DB·CI 에서만) |
