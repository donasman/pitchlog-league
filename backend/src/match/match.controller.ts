...
export class MatchController {
  ...
  list(@Query() q: MatchListQueryDto): Promise<MatchListDto> {
    return this.matches.list(q, parseLocale(q.locale));
  }
  ...
  detail(@Param('ref') ref: string, @Query('locale') localeQ?: string): Promise<MatchDto> {
    return this.matches.detail(ref, parseLocale(localeQ));
  }
  ...
  @ApiBadRequestResponse({ description: 'ref 형식 오류' })
  @ApiNotFoundResponse({ description: '경기가 없다' })
  fullDetail(@Param('ref') ref: string, @Query('locale') localeQ?: string): Promise<MatchFullDetailDto> {
    return this.matches.fullDetail(ref, parseLocale(localeQ));
  }
}
