-- 검색용 GIN trigram 인덱스 — Prisma 스키마로 표현되지 않는다 (SCHEMA_DESIGN 11장 확장)
--
-- 적용 방법:
--   npx prisma migrate dev --create-only --name search_indexes
--   → prisma/migrations/<ts>_search_indexes/migration.sql 끝에 이 파일 내용을 붙인다
--   → npx prisma migrate dev
--
-- 목적: /search API 의 이름 검색을 Seq Scan (~200 ms) → Bitmap Index Scan (< 10 ms) 로.
--   · 3자 이상: pg_trgm GIN 이 Bitmap Index Scan 을 태운다 (실측)
--   · 2자     : GIN trigram 은 3-그램 단위라 인덱스 못 씀 → btree text_pattern_ops 접두 인덱스로 커버
--
-- 컬럼 소유 계층은 L0·L1 (players/teams/competitions/localized_names).
-- 이 파일은 인덱스만 다룬다 — 데이터 변경 없음.
