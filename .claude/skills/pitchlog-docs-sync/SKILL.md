...
| 문서 | 고칠 곳 |
|---|---|
| `docs/NEXT_STEPS.md` | 0장 상태표 수치 · 1장 순서표(끝난 번호에 ✅) · 해당 장의 `[ ]`→`[x]` + 실측 결과 소절 · 14장 미결정(정해진 것은 `~~취소선~~` + ✅). **1장 순서표 번호는 안정 인덱스다. 끝난 항목은 취소선, 새 항목은 번호 없이 `~~—~~` 로 넣는다. 재번호하지 않는다 — `scripts/check-doc-refs.mjs` 가 CI 에서 막는다 (09-09, PR #37 에서 20곳 어긋남)** |
