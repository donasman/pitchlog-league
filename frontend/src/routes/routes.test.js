 ...
 * routes 설정 회귀 잠금
 *
 * `/competitions` 는 `CompetitionsPage` 를 삭제한 뒤 `/competitions/champions-league` 로
 * replace 리다이렉트한다 (feat/schedule-split · 2026-09-21 · UCL 을 기본 진입으로).
 * 이 잠금이 없으면 누가 `<Navigate>` 를 지우거나 다른 slug 로 바꿔도 CI 가 알려주지 못한다.
 *
...
describe('route: /competitions redirect', () => {
  it('redirects /competitions to /competitions/champions-league (replace)', () => {
