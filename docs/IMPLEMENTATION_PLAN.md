...
**8단계 전부 Mock 기준으로 완료됐다 (2026-09-06).** 이 문서의 역할은 끝났다.
그다음 — 실 API 전환 · 고정 시계 제거 · vitest — 은 `NEXT_STEPS.md` 11장이다.
`EmptyState` 의 `action` prop 하나가 1단계 감사에서 남아 있다 (컵 통계 탭 결손 · 홈 블록 결손에 필요).
