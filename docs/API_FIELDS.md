...
## 7. 다른 제공자 검토 — 단일 소스 유지 (2026-09-03)

선수 단위 xG가 없어서 다른 축구 통계 제공자를 검토했다. **결론: 추가하지 않는다.**

| 제공자 | 고급 통계 | 현재 시즌 | 판정 |
|---|---|---|---|
| Opta (Stats Perform) | 최상급 | ✅ | ❌ **기업 전용** — 공식 FAQ에 "designed for organisations rather than individual use". 공개 가격 없음, 영업 문의만 |
| StatsBomb Open Data | 슈팅 좌표 · xG · 360 | ❌ 과거만 | △ 26-27 시즌 없음. 학습·프로토타입용으론 최고 (출처 표기 조건) |
| Understat | xG · xA | ✅ | ❌ 공식 API 없음, 스크래핑 |
| FBref | 상세 지표 다수 | ✅ | ❌ API 없음, 스크래핑 차단 적극적 |
| Sportmonks | xG 포함 | ✅ | △ 유료. 실질적 유일 대안 |
| football-data.org | 기본만 | ✅ | ❌ xG 없음 |

### 추가하지 않는 이유

**1. 설계 원칙과 충돌한다.**
이 서비스는 "숫자는 백엔드가 확정하고 데이터 기준 시각과 근거를 함께 제공한다"를 원칙으로 한다.
소스가 둘이면 두 값이 어긋날 때(예: xG 0.33 vs 0.41) **무엇을 보여줄지 정해야** 하고,
어느 쪽을 택하든 "이 숫자를 믿어도 되는지 알려준다"는 약속이 흐려진다.

**2. ID 매칭 문제가 새로 생긴다.**
v2 설계는 팀·선수를 이름이 아니라 `api_team_id`로 연결하기로 했다(1-1, 6-7).
두 번째 소스를 붙이면 이름 기반 매칭이 다시 필요해진다 — 일부러 피한 방식이다.

**3. 수집 경로가 하나 더 는다.**
호출 예산, 실패 처리, 스케줄러가 각각 이중화된다.

### 예외 — 별도 실험으로만

슈팅 맵처럼 좌표가 필요한 것을 만들어보고 싶으면 **StatsBomb Open Data**가 무료다.
단 프로덕션에 섞지 않는다. 과거 경기만 있어서 "어떤 경기는 슈팅 맵이 있고 어떤 경기는 없는"
상태가 되기 때문이다.

### 재검토 조건

Sportmonks 도입을 다시 볼 만한 시점은 **선수 단위 xG가 제품의 핵심이 될 때**다.
현재 기획에서는 그렇지 않다 — xG는 경기 상세의 보조 지표다.
