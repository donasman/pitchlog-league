...
| **백엔드** | 🚧 Nest 12 · Prisma 29테이블 · API 클라이언트 · 배치 upsert · **L0**(17대회 · 82대회시즌 · 팀 1,888) · **조회 API 4개** · **L1 스쿼드**(155팀 · 선수 4,863 적재, 관문 테스트 21건 통과) · **L2-a 일정·라운드·순위**(코드 완료, 실 적재 전) |
...
09-07 에 계획한 것은 다 끝났다. **Phase 1 관문(스쿼드 diff)은 통과했고**, Phase 0 은
배포 PoC 하나만 남았다.

다만 **배포보다 화면이 먼저다.** 실 API 모드에서 12화면 중 사는 것은 대회·팀 2개뿐이고,
나머지는 경기·순위 데이터가 없어서 비어 있다. 그래서 L2-a(현재 시즌 수집)를 8장에서
떼어 먼저 한다. 다음 순서는 이렇다.

| # | 할 일 | 장 | 왜 이 순서인가 |
|---|---|---|---|
| 0 | **L2-a 실 적재 + 조회 API·프론트 연결** | 8 | 진행 중. 지금 12화면 중 2개만 산다 — 데이터가 없어서다 |
| 1 | **L1 두 번째 실행** | 9 | 며칠 뒤. 첫 실행은 전원 신규라 diff 판정이 한 번도 안 돌았다. 코드를 더 쌓기 전에 실 데이터에서 확인한다 |
| 2 | **배포 PoC** | 2 | Phase 0 의 마지막. 늦을수록 배포 제약이 코드에 반영될 기회를 잃는다 |
| 3 | **L1 #10 감독 이력** | 9 | diff 구조를 그대로 재사용한다 — L1 이 검증된 지금이 가장 싸다 |
| 4 | **L1 #9·#11** | 9 | 선수 프로필(`/players?id=`) · 부상자. 독립적이라 순서 자유 |
| 5 | **백업 자동화 · 사본 이중화 · 리허설 재실행** | 4 | **백필 착수 직전.** 백필은 8~12일 무인 실행이다 |
| 6 | **L2-b 백필 실행** | 8 | 5번이 끝나야 시작한다. 여기서 UCL 예선팀도 걷어낸다 |

병렬 가능: 2번은 1·3·4와 무관하다. 7단계 화면 작업(대진표·시즌 선택기)도 언제든 끼워넣을 수 있다.

**되돌리기 어려운 순서는 5→6 하나다.** 나머지는 바꿔도 된다.
...
`INGESTION_STRATEGY.md` 2-2 · 5장. **8-a(현재 시즌)와 8-b(백필·녹아웃)로 나눈다.**
09-07 에 "화면이 하나도 안 나온다" 가 순서를 바꿨다 — 배포 PoC 보다 먼저 화면에 데이터를 채운다.
...
- [x] **소유권 경계** — `has_*` · `detail_checked_at` · `data_version` · `tie_id` · `leg` 은
      건드리지 않는다. L3~L5 와 8-b 의 것이고, 재실행이 저쪽 진행 상태를 지우면 안 된다
- [x] e2e 6건 — 컷 앞 라운드 미저장 · ordinal 보존 · 목록 밖 라운드/DB 없는 팀 건너뜀 ·
      멱등 + 스코어 갱신 · 소유권 경계 · 고아 행 없음
- [ ] 실 적재 1회 — 컷이 UCL 리그페이즈에서 잡히는지 확인한다
- [ ] 조회 API 확장(경기·순위) + 프론트 연결

UCL 의 "1부 팀 집합" 은 `top_flight_competition_id` 가 없어 **추적 5대 리그 참가팀 합집합**을
쓴다. 5대 리그 팀은 예선을 치르지 않으므로 컷은 리그페이즈에서 잡힌다 — 실 적재로 확인한다.

### 8-b. 백필과 녹아웃 — 8~12일

지금 못 하는 이유가 있다. **직전 시즌 템플릿이 없어 대진표 골격을 만들 수 없고**,
UCL 녹아웃은 2027년 2월이라 **검증할 실 데이터가 없다.** 만들어도 검증이 안 된다.

- [ ] `knockout_ties` · `bracket_slots` — 1·2차전 묶기, 대진표 슬롯 (8-2)
- [ ] 국내 컵 6개 + 슈퍼컵 5개
- [ ] **최신 시즌부터 역순.** 1일차에 2026 시즌이 끝나면 사이트를 열 수 있다
- [ ] BullMQ Worker 승격 — 백필이 Redis 를 실제로 도입하는 첫 작업
- [ ] `backfill_jobs` 재시작 지점
- [ ] 백필은 비경기일에 몰고 경기일에는 폴링을 우선한다
- [ ] **UCL 예선팀을 수집 대상에서 걷어낸다** ★ — 아래
...
예선팀과 리그페이즈 팀을 구분하려면 **라운드 데이터가 필요하다** — 8-a 가 그걸 넣는다.
