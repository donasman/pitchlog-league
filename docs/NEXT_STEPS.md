...
- [ ] **정체불명 테이블 `equipments` 정리** — 리허설에서 `public` 테이블이 31개로 나와
      추적해 보니 `equipments` 였다. Prisma 스키마에 없고, OID 18251 로 전체에서 가장 높아
      (Prisma 마지막이 `localized_names` 18102) **init 마이그레이션 이후에 따로 만들어진 것**이다.
      내용을 확인하고, 쓰지 않는 것이면 지운다 — 스키마에 없는 테이블이 DB 에 있으면
      Prisma 가 드리프트로 본다
