...
- [ ] **UCL 조별리그 시즌 순위표** (09-08 발견, 8-b) — 2022·2023 은 8조 × 4팀이라
      `group_name` 으로 표를 쪼개지 않으면 **rank 1 인 팀이 8개**로 보인다. 2024 이후는 단일 36팀.
      백엔드 수정은 없다 — 데이터는 맞게 들어와 있다
  - [ ] `zoneOf` 오탐 — 규칙 `Champions League + (Qualif|Play)` 가
        `Promotion - Champions League (Play Offs: 1/8-finals)` 에 걸린다.
        조 1위가 `champions_league_playoff` 로 잡히고 그 범례 라벨은 빈 문자열이다
  - [ ] **시즌 선택기 노출** — `backfill_jobs.phase = DONE` 인 30대회시즌만
        (`INGESTION_STRATEGY` 5-4). 8-b 가 백엔드 쪽은 채웠다
