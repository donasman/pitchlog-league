...
5대 리그 합이 96팀인데 화면 6대회 전체는 **155팀** — 59팀이 UCL 에서만 왔고 상당수가
예선 탈락팀이다. Petrocub(몰도바) · Drita(코소보) · Lincoln Red Imps(지브롤터) ·
ML Vitebsk(벨라루스) 같은 클럽으로, 우리가 수집할 189경기에 한 번도 나오지 않는다.

**지금은 두지 않고 여기서 정리한다.** 비용이 주 55콜·수백 KB 라 당장은 무시할 수준이고,
예선팀과 리그페이즈 팀을 구분하려면 **라운드 데이터가 필요하다** — 그게 이 단계다.
"5대 리그 참가팀만" 으로 좁히면 벤피카·아약스·PSV 같은 정당한 리그페이즈 팀이 빠진다.

⚠ **L5·L6(선수 시즌 통계) 전에는 반드시 끝낸다.** 지금은 선수 1,700명이 놀고 있을 뿐이지만,
