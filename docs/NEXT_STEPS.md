...
| 4 | **서버화** — 스케줄러 잡(4-b-1 백필 워커 + /status 스냅숏 · **4-b-2 L2 매일 + L1 매주는 백필-2 통과 후**) · **EC2(systemd) + Vercel(`/api` rewrite)** · 백업 자동화(EC2 systemd 타이머 `pg_dump` → S3) + 리허설. 세 판으로 쪼갠다: 4-a 배포 → 4-c 백업 자동화(관문) → 4-b-1 워커 → **백필-2 나머지 4시즌 무인** | 2·4·8-c | 43,260콜 · **8일** (`INGESTION_STRATEGY.md:175` 산식) | 외부 접속 · 5시즌 경기 상세 |
