...
| **백엔드** | 🚧 Nest 12 · Prisma 29테이블 · API 클라이언트 · 배치 upsert · **L0 실 적재 완료**(17대회 · 82대회시즌 · 고유 팀 1,888) · **조회 API 4개** · **L1 스쿼드 diff**(관문 테스트 13건 통과, 실 적재 전) |
...
- [ ] **L1 첫 실 적재** — `npm run ingest -- l1`. 155팀 · 콜 155. 결과의 `skipped` 를 본다
      (빈 응답·급감으로 건너뛴 팀이 많으면 가드 임계값을 다시 본다)
...
- [x] ~~#7 스쿼드 스냅샷~~ ✅ `npm run ingest -- l1` — 화면 6대회 현재 시즌 참가팀(실측 155)
