...
> **12대회(컵 6개 추가) × 5시즌**으로 커졌으며, 예산은 추정이 아니라 실측 기반으로 다시 계산했다.
...
> 아래 표는 "EPL로 시작해 Phase 4에서 다중화"를 전제하지만, 실측 결과 **12대회 × 5시즌**이
> Phase 1부터 범위가 됐다. 컵 6개는 컷오프 덕에 시즌당 상세 222경기라 비용이 작고,
> 대회를 나중에 늘리면 스키마와 화면을 두 번 만들게 된다.
