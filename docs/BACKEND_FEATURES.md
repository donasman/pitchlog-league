...
> 대회 수는 6개에서 **12개(컵 6개 추가)** 로, 범위는 **최근 5시즌**으로 확정됐다.
...
- 구현됨: `GET /api/competitions` · `/api/competitions/:ref` · `/api/teams?competition=&season=` · `/api/teams/:ref` · **`/api/matches` · `/api/matches/:ref` · `/api/standings` (09-07)** · **`/api/players/:ref` · `/api/stats/scorers` · `/api/stats/assisters` (09-08)**. 나머지는 데이터가 들어오는 계층과 같이.
